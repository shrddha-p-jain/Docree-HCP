# -*- coding: utf-8 -*-
"""2. Modelling Final.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1CxaW236pS8l9Sq0AbJzVLRXCX7LHm6ui
"""
...
train = train.drop(['DEVICETYPE', 'PLATFORM_ID', 'BIDREQUESTIP', 'USERPLATFORMUID', 'USERCITY', 'USERZIPCODE', 'USERAGENT', 'PLATFORMTYPE', 'CHANNELTYPE',
       ...
       'Classification', 'TAX1', 'TAXONOMY_Pred', 'IP1', 'IP2', 'IP3', 'IP4', 'device_brand', 'device_family', 'device_model', 'os', 'ua'],axis=1,inplace = True)
...
s = setup(train, target = 'is_h', session_id = 101,train_size=0.80,imputation_type = 'iterative', fold_shuffle=True,
...
train1 = pd.read_parquet(r'train_mee_tf.parquet')
test1 = pd.read_parquet(r'test_mee_tf.parquet')
train_p = train1[train1.IS_HCP==1]['USERPLATFORMUID']
test_p = test1[test1.IS_HCP==1]['USERPLATFORMUID']
puid = pd.concat([train_p,test_p])

pred0 = predict_model(stack5,data_test_final_me_tf)
pred0.to_parquet(r'best0.parquet')
pred0 = pd.merge(pred0,data_test_final[['ID','USERPLATFORMUID']],on = 'ID')
pred0['IS_HCP'] = np.where(pred0.USERPLATFORMUID.isin(puid),1,pred0.prediction_label)
pred0[['ID','IS_HCP']].to_csv(r'submission_final.csv',index = False)
